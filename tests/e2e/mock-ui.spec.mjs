...
test('renders the credential dashboard on wide and narrow viewports', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 });
  await page.goto('/');

  await expect(page.getByRole('heading', { name: '資格の更新時期を、ひと目で。' })).toBeVisible();
  await expect(page.getByText('Azure Administrator Associate', { exact: true }).first()).toBeVisible();
