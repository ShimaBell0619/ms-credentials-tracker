...
The initial UI baseline is established by PR #5. The current milestone introduces the first credential-domain logic and validates browser-local Microsoft Learn Transcript PDF import before server-side persistence, authentication, or Microsoft API integration is selected.
...
- server-side API or database for credential import
