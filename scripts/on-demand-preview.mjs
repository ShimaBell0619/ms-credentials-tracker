...
async function publish() {
  ...
  if (observed) {
    git(['push', 'origin', `${syntheticSha}:${remoteRef}`, `--force-with-lease=${remoteRef}:${observed}`]);
  } else {
...
async function notify() {
  ...
  await githubApi(`repos/${repo}/issues/${prNumber}/comments`, { method: 'POST', body: { body } });
}
...
async function cleanup() {
  ...
  git(['push', 'origin', `:${remoteRef}`, `--force-with-lease=${remoteRef}:${observed}`]);
}
