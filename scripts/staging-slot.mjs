...
export function createGitHubClient({ token, repository, apiUrl = 'https://api.github.com', fetchFn = fetch }) {
  ...
  return {
    ...
    async listManualRuns(workflowFile) {
      const payload = await request(`${repoPath}/actions/workflows/${encodeURIComponent(workflowFile)}/runs?event=workflow_dispatch&branch=${encodeURIComponent(MAIN_BRANCH)}&per_page=100`);
...
async function publishStaging(client, repository) {
  ...
  if (await isSuperseded(client, workflowFile, requestRunId)) {
    writeSummary(['## Fixed Staging', '', `Request run ${requestRunId} was superseded by a newer main-branch request.`, 'No Staging ref change was made.']);
    return;
  }
  ...
  let reused = false;
  if (existingParent === sourceSha && existingTree === sourceTree && sourceMarkers(existingMessage, prNumber, sourceSha)) {
    reused = true;
  } else {
    const message = [`Foundation Fixed Staging for PR #${prNumber}`, '', `Foundation-Fixed-Staging-PR: ${prNumber}`, `Source-PR-HEAD: ${sourceSha}`].join('\n');
    ...
    if (git(['show', '-s', '--format=%P', syntheticSha]) !== sourceSha) throw new Error('Fixed Staging synthetic commit must have source A as its single parent.');
    if (git(['show', '-s', '--format=%T', syntheticSha]) !== sourceTree) throw new Error('Fixed Staging synthetic commit tree must equal source A tree.');
    git(['diff', '--quiet', sourceSha, syntheticSha]);

    if (await isSuperseded(client, workflowFile, requestRunId)) {
      writeSummary(['## Fixed Staging', '', `Request run ${requestRunId} was superseded before mutation.`, 'No Staging ref change was made.']);
      return;
    }

    pr = await client.getPullRequest(prNumber);
    if (validateDeployablePullRequest(pr, repository) !== sourceSha) throw new Error('PR HEAD changed before Staging mutation; request Fixed Staging again.');
  ...
  try {
    ...
    console.warn(`Fixed Staging is ready, but PR feedback could not be posted: ${error instanceof Error ? error.message : String(error)}`);
  }

  writeSummary(['## Fixed Staging ready', '', `- PR: #${prNumber}`, `- Source A: \`${sourceSha}\``, `- Synthetic B: \`${syntheticSha}\``, `- URL: ${stagingUrl}`, `- Reused: ${reused}`]);
}
...
async function cleanupStaging(client, repository) {
  ...
  if (!stagingOwnershipMatches(message, prNumber)) {
    writeSummary(['## Fixed Staging cleanup skipped', '', `Closed PR: #${prNumber}`, 'Current Staging is owned by another source or is already reset.']);
    return;
  }
  ...
  try {
    ...
    if (current !== observed) {
      writeSummary(['## Fixed Staging cleanup skipped', '', `Closed PR: #${prNumber}`, `Staging changed concurrently to \`${current}\`; the newer occupant was preserved.`]);
      return;
    }
    throw error;
  }
  ...
  writeSummary(['## Fixed Staging cleaned up', '', `Closed PR: #${prNumber}`, `Staging reset to main SHA \`${mainSha}\`.`]);
}
