...
export function shouldCleanupStaging(currentStagingSha, closedPrHeadSha) {
  return assertSha(currentStagingSha, 'current Staging SHA') === assertSha(closedPrHeadSha, 'closed PR HEAD SHA');
}
...
export async function deployToStaging({ client, repository, prNumber, workflowFile, runId, githubRef, runGit = defaultRunGit }) {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    if (runGit(buildStagingPushArgs(targetSha, expectedStagingSha))) {
      ...
      if (verified !== targetSha) throw new Error(`Staging verification failed: expected ${targetSha}, found ${verified}.`);
...
export async function cleanupStaging({ client, repository, closedPrNumber, closedPrHeadRepo, closedPrHeadSha, runGit = defaultRunGit }) {
  ...
  const verified = await client.getRef(STAGING_BRANCH);
  if (verified !== mainSha) throw new Error(`Staging cleanup verification failed: expected ${mainSha}, found ${verified}.`);
...
async function main() {
  ...
  if (command === 'deploy') {
    ...
    try {
      ...
      console.warn(`Staging was updated, but the PR comment could not be posted: ${error instanceof Error ? error.message : error}`);
    }
  ...
  if (command === 'cleanup') {
    ...
    const result = await cleanupStaging({
      ...
      repository,
      closedPrNumber,
      closedPrHeadRepo: requireValue(process.env.CLOSED_PR_HEAD_REPO, 'CLOSED_PR_HEAD_REPO'),
